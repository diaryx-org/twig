...
pub fn build(b: *std.Build) void {
    ...
    // ---- version and the pre-release gate ----------------------------------
    //
    // The maintenance side of the build. Cutting a release is not here — see
    // the note at the end of this function, and docs/RELEASING.md.
    //
    // These are `has_side_effects`: they ask git and the filesystem questions
    ...
    // Cutting a release is not a build step any more. It is `release <command>`,
    // from diaryx-org/devtools, configured by `.config/release.toml` — the same
    // tool twig, prov, leaf, flower, and the historica repos all cut releases
    // with. `verify = "zig build check"` in that config is how `check` above
    // stays twig's own pre-release gate, and `verify_after_bump = true` is how
    // it still runs after the bump rather than before it, since
    // `sync-version-check` judges the versions the release is writing. See
    // docs/RELEASING.md.
}
